...
class CHelloClassFactory : ComObject, IClassFactory
{
    ...
    extern (Windows):
        ...
        // IUnknown members
	HRESULT QueryInterface(IID* riid, LPVOID *ppv)
	{
	...
	HRESULT CreateInstance(IUnknown pUnkOuter, IID* riid, LPVOID * ppvObj)
	{
	    ...
	    *ppvObj = null;
	    ...
	    if (null != pUnkOuter && memcmp(&IID_IUnknown, riid, IID.sizeof))
...
HRESULT DllRegisterServer()
{
    ...
    SetKeyAndValue("Hello1.0", "CLSID", szID);

    //Create VersionIndependentProgID keys
    SetKeyAndValue("Hello", null, "Hello Object");
    SetKeyAndValue("Hello", "CurVer", "Hello1.0");
    SetKeyAndValue("Hello", "CLSID", szID);

    //Create entries under CLSID
    SetKeyAndValue(szCLSID, null, "Hello Object");
    SetKeyAndValue(szCLSID, "ProgID", "Hello1.0");
    SetKeyAndValue(szCLSID, "VersionIndependentProgID", "Hello");
    SetKeyAndValue(szCLSID, "NotInsertable", null);

    GetModuleFileNameA(g_hInst, szModule, szModule.length);

    SetKeyAndValue(szCLSID, "InprocServer32", szModule);
    return NOERROR;
}
...
HRESULT DllUnregisterServer()
{
    ...
    strcpy(szTemp, szCLSID);
    strcat(szTemp, "\\");
    strcat(szTemp, "ProgID");
    RegDeleteKeyA(HKEY_CLASSES_ROOT, szTemp);

    strcpy(szTemp, szCLSID);
    strcat(szTemp, "\\");
    strcat(szTemp, "VersionIndependentProgID");
    RegDeleteKeyA(HKEY_CLASSES_ROOT, szTemp);

    strcpy(szTemp, szCLSID);
    strcat(szTemp, "\\");
    strcat(szTemp, "NotInsertable");
    RegDeleteKeyA(HKEY_CLASSES_ROOT, szTemp);

    strcpy(szTemp, szCLSID);
    strcat(szTemp, "\\");
    strcat(szTemp, "InprocServer32");
    RegDeleteKeyA(HKEY_CLASSES_ROOT, szTemp);

    RegDeleteKeyA(HKEY_CLASSES_ROOT, szCLSID);
    return NOERROR;
}
